      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(150px,1fr))', gap: 12 }}>
        {loading ? (
          ...
          (items.slice ? items.slice(0, 12) : items).map(it => (
            <div key={it.id} className="card" style={{ background: 'var(--bg-tertiary)', color: 'var(--text-primary)', padding: 8, borderRadius: 6, display: 'flex', flexDirection: 'column', boxShadow: 'var(--shadow)' }}>
              {it.poster_path ? (
              ...
              <div style={{ marginTop: 8 }}>
                <button aria-label={`Voir détails ${it.title || it.name}`} onClick={() => onSelect && onSelect({ id: it.id, type: 'movie', title: it.title || it.name, year: (it.release_date || it.first_air_date || '').slice(0, 4), poster: it.poster_path })}>Détails</button>
                <button aria-label={`Ajouter ${it.title || it.name}`} onClick={() => {
                ...
                }} style={{ marginLeft: 8 }}>Ajouter</button>
              </div>
            </div>
          ))
        )}
